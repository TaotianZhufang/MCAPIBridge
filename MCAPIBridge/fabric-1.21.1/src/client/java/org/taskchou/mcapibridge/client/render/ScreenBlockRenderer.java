...
public class ScreenBlockRenderer implements BlockEntityRenderer<ScreenBlockEntity> {
    ...
    public void render(ScreenBlockEntity entity, float tickDelta, MatrixStack matrices,
        ...
        // TR (Top Right)
        buffer.vertex(matrix, 0.5f, 0.5f, 0).color(255,255,255,255).texture(u1, v1).light(0xF000F0);
        // BR (Bottom Right)
        buffer.vertex(matrix, 0.5f, -0.5f, 0).color(255,255,255,255).texture(u1, v2).light(0xF000F0);
        // BL (Bottom Left)
        buffer.vertex(matrix, -0.5f, -0.5f, 0).color(255,255,255,255).texture(u2, v2).light(0xF000F0);
        // TL (Top Left)
        buffer.vertex(matrix, -0.5f, 0.5f, 0).color(255,255,255,255).texture(u2, v1).light(0xF000F0);
