...
public class IODataState extends PersistentState {
    ...
    private static final Type<IODataState> TYPE = new Type<>(
            IODataState::new,
            IODataState::fromNbt,
            null
    );
    ...
    public static IODataState getServerState(MinecraftServer server) {
        ...
        return stateManager.getOrCreate(TYPE, "mcapibridge_io");
    }
    ...
    public NbtCompound writeNbt(NbtCompound nbt) {
        ...
        blocks.forEach((id, positions) -> {
            ...
            for (GlobalPos gp : positions) {
                ...
                p.putInt("x", gp.getPos().getX());
                p.putInt("y", gp.getPos().getY());
                p.putInt("z", gp.getPos().getZ());
                p.putString("dim", gp.getDimension().getValue().toString());
                posList.add(p);
            }
    ...
    public static IODataState fromNbt(NbtCompound nbt) {
        ...
        for (int i = 0; i < rootList.size(); i++) {
            ...
            for (int j = 0; j < posList.size(); j++) {
                ...
                RegistryKey<World> dimKey = RegistryKey.of(RegistryKeys.WORLD, new Identifier(dim));
